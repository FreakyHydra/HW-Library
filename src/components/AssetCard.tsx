import { ArrowUpRight, Boxes, MapPin, MoreHorizontal, Pin } from 'lucide-react';
...
export function AssetCard({ asset, featured = false }: { asset: LibraryAsset; featured?: boolean }) {
  ...
  return (
    <article className={`asset-card tone-${asset.visualTone} ${featured ? 'asset-card--featured' : ''}`}>
      <Link className="asset-card__visual" to={`/asset/${asset.id}`} aria-label={`Open ${asset.name}`}>
      ...
      <div className="asset-card__body">
        <div className="asset-card__title-row">
          <div><span className="asset-card__source">{sourceLabels[asset.sourceType]}</span><h3><Link to={`/asset/${asset.id}`}>{asset.name}</Link></h3></div>
          <button className="icon-button" aria-label={`More actions for ${asset.name}`}><MoreHorizontal size={19} /></button>
        </div>
        <p>{asset.summary}</p>
        ...
        <div className="asset-card__footer">
          ...
          <Link className="card-open" to={`/asset/${asset.id}`} aria-label={`View ${asset.name}`}><ArrowUpRight size={18} /></Link>
        </div>
      </div>
    </article>
  );
